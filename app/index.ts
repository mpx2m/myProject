...
const getBundles = async () => {
  const bundles = await fetchJSON('/elasticapp/api/list-bundles');
  if (bundles.error) {
    throw bundles.error;
  }
  return bundles;
...
const deleteBundle = async (bundleId) => {
  try {
    ...
    const url = `/elasticapp/api/bundle/${encodeURIComponent(bundleId)}`;
...
const addBundle = async (name) => {
  try {
    ...
    // Add the new bundle.
    const url = `/elasticapp/api/bundle?name=${encodeURIComponent(name)}`;
...
const getBundle = bundleId =>
    fetchJSON(`/elasticapp/api/bundle/${encodeURIComponent(bundleId)}`);
...
const viewBundle = async (id, bundle) => {
  try {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      const url = `/elasticapp/api/search/books/title/${encodeURIComponent(input.value)}`;
    ...
    input.addEventListener('input', async (event) => {
      ...
      const url = `/elasticapp/api/suggest/title/${encodeURIComponent(last)}`;
...
const addBookToBundle = async (bundleId, bookId, title) => {
  try {
    ...
    await fetchJSON(`/elasticapp/api/bundle/${encodeURIComponent(bundleId)}` +
...
const getBook = bookId => fetchJSON(`/elasticapp/api/book/${encodeURIComponent(bookId)}`);
...
const showView = async () => {
  ...
  let [weburl, viewall] = window.location.href.split('#')
  const [viewRaw, ...params] = viewall.split('/')
  let view = "#" + viewRaw
...
//返回 #welcome
const backWelcome = () =>{
  let [websiteurl, ...other] = window.location.href.split('elasticapp')
  window.location.href = websiteurl+"elasticapp/#welcome"
}
...
(async () => {
  ...
  //返回 #welcome
  showView().catch(err => backWelcome());
