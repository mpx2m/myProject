...
function getScores() {
  ...
  window.location = "/personalitytest/testscore" + queryString;
}

document.getElementById("getScore").addEventListener('click',getScores,false)

let scoreInput= document.querySelectorAll("div[data-questionNumber] input")

for(let i=0; i<scoreInput.length; i++) {
    scoreInput[i].addEventListener('click',e => setAnswer(e.target),false)
}
